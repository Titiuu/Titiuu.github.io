...
AI coding 最危险的时刻，往往不是它写不出代码，而是它写得太顺了。
...
`grill-me`、OpenSpec 和 Superpowers 都在尝试解决这个问题，只是它们施加约束的位置和强度不同。本文不准备选出一个“最佳工具”。真正重要的不是安装哪一个项目，而是建立一个习惯：

> 在允许 AI 执行之前，先让它帮助我们把问题想清楚。

## 问题不在于 AI 不会思考，而在于默认流程鼓励它行动
...
AI coding 时代之前形成的项目，最危险的问题不是没有一份漂亮 spec，而是大量真实约束只存在于代码、测试、配置、部署脚本和人的经验中。
...
老项目的第一原则不是“先设计理想系统”，而是“先建立对现实系统足够准确的模型”。
...
需要警惕的不是“流程太少”，而是把某个工具变成条件反射。工具的价值来自它降低了错误与返工成本；当流程成本高于风险成本时，就应该减轻甚至跳过。

## 结语：晚一点写代码，可能更早交付

grill-me、OpenSpec 和 Superpowers 的共同点，不是它们都能让 AI 生成更多 Markdown，而是它们都在尝试推迟未经确认的行动。
...
AI coding 的关键竞争力，最终不只是让 AI 写得更快，而是知道什么时候还不该让它写。
