...
Dream 是 nanobot 最核心的记忆维护机制。它不是一个传统的 RAG 或向量搜索方案，而是一个**定期唤醒 LLM、给它受限的文件编辑工具、让它自己读写长期记忆文件**的自治 Agent。
...
这套规则的精妙之处在于：**它不是让 LLM "看着办"，而是给出了可执行的判别标准**。

### 2.4 Dream 的工具限制：只给文件编辑能力

Dream 执行时，纳米机器人给它分配的不是完整的工具集，而是一个被严格裁剪的注册表：
...
nanobot workspace 里有一组上下文文件，它们不是零散的配置项，而是一个按职责划分的最小框架。我在之前的笔记里已经详细梳理过各文件的用途，这里只从"专用 Bot 改造"的角度重新组织。
...
从工程角度看，nanobot 最值得学习的地方可能不是某个具体功能的实现，而是它在"让 LLM 参与系统自身的长期维护"这件事上的设计决策——把文件编辑能力交还给 LLM，把状态感知能力暴露给 LLM，然后用协议和边界保证安全性。
