...
Hermes Agent 的差异点就在这里。它的“自进化”不是训练模型权重，也不是让某个隐藏状态在模型内部神秘增长，而是把任务经验反写进**外部认知资产**，再让这些资产在后续任务中重新进入上下文。
...
所以本文只回答三个问题：Hermes 到底自己改写什么？它怎么判断和执行改写？这些改写后的内容什么时候重新进入上下文？
...
长期运行的个人 agent 遇到的真实问题并不是“模型会不会背更多知识”，而是四件更具体的事。
...
自进化系统最怕的不是“不学习”，而是“什么都学”。如果写入判断差，长期记忆会变成杂乱笔记本，skills 会变成一堆搜索不到的会话残片。
...
因此，Hermes 的关键不在于“会写文件”这个动作本身，而在于分层：什么进入常驻记忆，什么留在历史库，什么沉淀为 skill，什么交给 curator 合并和归档。

回到标题，Hermes “自己改写”的不是大模型，而是自己的记忆、技能库和用户模型；这些内容再通过 prompt 注入、按需 skill 加载、历史检索和可选的 Honcho context 回到后续推理中。
