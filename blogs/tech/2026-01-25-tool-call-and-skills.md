...
这里最重要的事实是：**tool call 不是工具执行本身，而是模型给 runtime 的结构化请求**。
...
所以 CLI 的价值不是“比 MCP 更新”，而是它把外部能力暴露成了一种模型已经很熟的交互语言。
...
所以 Skills 更像是“agent 能力的包管理和上下文管理机制”。它可以调用普通文件、脚本、MCP 工具、CLI 命令，也可以只是纯说明文档。它的核心不是协议，而是任务能力组织。
