...
并行的作用是减少等待时间，并让每个子任务有更专注的上下文，而不是增加表面复杂度。
...
这里 subagent 的价值主要来自**上下文独立**，不代表它更聪明。它没有背负实现过程中的错误假设，因此更容易发现问题。
...
这里的关键在运行时。没有运行时支持，主 agent 输出再漂亮的“我启动一个子 agent”也只是文本。
...
启动 subagent 并不难，难的是围绕它建立完整工程约束：
