...
这个结构的价值在于减少分叉，少写一个文件只是附带效果。当 `CLAUDE.md` 和 `AGENTS.md` 里同一条规则写得不一样时，agent 的行为会变得不可预测，团队也很难判断到底哪份才是准的。
...
这就是 context engineering 的核心：决定哪些信息应该在什么时机、以什么优先级、用多少 token 进入上下文，而不是全部塞进去。
...
对使用者来说，比背某个工具的内部字段更重要的是一条原则：越晚进入上下文、越靠近当前任务的指令，越应该具体。
...
如果这种冲突经常出现，通常说明父目录写得太具体。应把局部规则下沉到对应子目录，让根目录只保留全局规则，避免让 agent 每次猜测。
