...
这个说法没有错，但只说到了最外层。真正跑过 coding agent、个人 agent 或多入口 agent 后，会发现上下文压缩不是一个 prompt 技巧，而是 agent runtime 里一套完整的状态管理系统。
...
粗估的问题不只是“不准”，而是会影响压缩时机。触发太晚会请求失败，触发太早会频繁丢上下文、破坏缓存、增加成本。
...
上下文压缩做得好，不是因为 summary 写得漂亮，而是因为这两条线分开了。
