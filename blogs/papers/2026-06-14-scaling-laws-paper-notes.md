...
论文没有提出新架构，核心贡献是用大规模实验给出一组经验规律：语言模型的 loss 会随着参数量、数据量和训练计算量呈现平滑的幂律下降。这让研究者可以据此外推效果和规划训练预算。
...
Scaling law 论文给出的公式并非永远正确，它更持久的影响是改变了大模型研发的思维方式。
...
因此，Kaplan 论文留下的不只是某个固定指数，还有一套方法：用系统实验拟合缩放规律，再用规律指导大模型资源分配。
...
这也是 Chinchilla 能修正 Kaplan 最优数据/参数配比的原因：扩大实验范围并改变实验设计后，可以得到更合适的工程规律，并不意味着 Kaplan 论文错得离谱。
