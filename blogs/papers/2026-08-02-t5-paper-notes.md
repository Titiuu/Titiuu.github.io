...
T5 的关键意义，不是证明 encoder-decoder 比 GPT-2 更优，而是通过 text-to-text 统一任务表示与训练目标，并以大规模系统实验表明：**统一的模型接口和针对任务的微调并不矛盾，二者共同构成了一条可扩展的迁移学习路线。**
...
T5 统一的是表达问题的方式；微调保留的是解决具体问题的能力。正是这两者的结合，让预训练模型从共享表示进一步走向了可复用的通用任务框架。
