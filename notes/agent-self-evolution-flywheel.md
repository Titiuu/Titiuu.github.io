...
> 标题：一篇讲透Agent自进化飞轮怎么搭：评测→记忆→落地→控制
> 作者：yannisyang、ethanytzhou（腾讯技术工程「腾讯程序员」公众号）
> 说明：个人参考用全文存档，正文照录原文抓取结果，仅去除了微信页面杂质。

---

# 一篇讲透Agent自进化飞轮怎么搭：评测→记忆→落地→控制

Original 腾讯程序员 腾讯程序员 腾讯技术工程
...
**第三层：Model 进化——改模型参数本身。** 通过 Agent RL、自对弈微调将经验写入权重。持久性最强，但成本最高、风险最大。目前仍在研究阶段——[MiniMax M2.7](https://mp.weixin.qq.com/s?__biz=MzI4MDYzNzg4Mw==&mid=2247579992&idx=3&sn=7b5386dd465072b2dca370376dc14b46&scene=21#wechat_redirect) 和业界多个团队的调研都在探索这条路，但距离生产落地还有距离。
...
**Step 3：需要时才钻取底层。** 只有Agent执行中遇到困惑或需要证据时，才通过node\_id回溯到原始对话——平时不加载。
...
**一个反直觉的发现**（[Anthropic Dreaming 2026.5](https://cloud.tencent.com/developer/article/2668950)）：**记忆建模为文件系统，让Agent用Bash/grep检索管理，比硬造专用API更有效。** Agent本就擅长操作文件——ls/grep/cat是它最自然的交互方式。

**符号化压缩**是另一个值得关注的方向：用Mermaid编码任务状态，完整日志卸载到外部，Agent在符号图上推理、需要时通过node\_id检索——实测**Token节省超60%，任务通过率显著提升。**
...
**为什么完全放手是危险的？**[Anthropic《When AI builds itself》2026.6](https://www.anthropic.com/institute/recursive-self-improvement)警告：AI在自主迭代修改自身配置时，可能导致行为发生**隐蔽且累积的偏移**——即使每次单独的改动看起来合理，累积起来可能已经偏离初始意图。在Harness层同样成立——而且由于飞轮在自动转，**一个错误被放大的速度和飞轮转动的速度成正比。**
